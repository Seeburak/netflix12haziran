...
def profileLogin(request,pid):
   profile_list = Profile.objects.filter(user=request.user) # kullanılan tüm profilleri
...
#dizi ve Film sıralama / girişli profilin kayıt tutulması
     
def profilkim(request):

   context={
      "profilkim":Profile.objects.filter(islogin=True),
      }
   return render(request,"profilkim.html",context)
...
def filmler(request):
   ...
   context={
        "profilkim":Profile.objects.filter(user=request.user,islogin=True),
        "filmler":Video.objects.filter(Category=1)
    }
...
def filmdizi(request):
    context={
        "filmdizi":Video.objects.order_by('-content')
    }
